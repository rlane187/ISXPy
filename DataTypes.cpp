...
bool PyType::ToText(LSOBJECTDATA ObjectData, char *buf, unsigned int buflen)
{
	...
	strcpy_s(buf,buflen,"ISXPy");
	return true;
}

bool PyType::GetMember(LSOBJECTDATA ObjectData, PLSTYPEMEMBER pMember, int argc, char *argv[], LSOBJECT &Object)
{
	...
	switch((PyTypeMembers)pMember->ID)
	{
	case RetrieveData:
...
bool PyType::GetMethod(LSOBJECTDATA &ObjectData, PLSTYPEMETHOD pMethod, int argc, char *argv[])
{
	...
	switch((PyTypeMethods)pMethod->ID)
	{
	case PerformAction:
